...
::: {.callout-tip}
**APA style:** `style=apa` loads `biblatex-apa`, which implements **APA 7th edition** (2019) and requires `backend=biber`. For full APA *manuscript layout* (double-spacing, running head, author note) required by APA journals, also load the `apa7` document class alongside `biblatex-apa`.
:::
...
## Linguistics: Interlinear Glosses — `gb4e` {.smaller}
...
The `avm` package typesets attribute-value matrices used in HPSG, LFG, and related frameworks:
...
**Alternative: `forest` with AVMs**

Some authors use `forest` or raw `tikz` for AVMs when layout control is critical. For standard HPSG work, `avm` is the established choice.
:::
::::
