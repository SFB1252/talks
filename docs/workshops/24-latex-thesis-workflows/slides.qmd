...
## Bibliography: Choosing a Backend and Style Package {.smaller}
...
```latex
% Option 1: biblatex + biber (recommended for new work)
\usepackage[backend=biber, style=apa]{biblatex}
\addbibresource{references.bib}   % path to your .bib file

% Option 2: natbib + BibTeX (journal template compatibility)
\usepackage[round, authoryear]{natbib}
% no \addbibresource — BibTeX is told the file via \bibliography{references}
% at the end of the document instead

% Option 3: biblatex + BibTeX fallback
\usepackage[backend=bibtex, style=apa]{biblatex}
\addbibresource{references.bib}

% Option 4: apacite — AVOID for new work (APA 6th only, unmaintained)
\usepackage{apacite}
% bibliography printed with: \bibliography{references}
```
...
::: {.callout-tip}
**APA style:** `style=apa` loads [`biblatex-apa`](https://ctan.org/pkg/biblatex-apa), which implements **APA 7th edition** (2019) and requires `backend=biber`. For full APA *manuscript layout* (double-spacing, running head, author note) required by APA journals, also load the [`apa7`](https://ctan.org/pkg/apa7) document class alongside [`biblatex-apa`](https://ctan.org/pkg/biblatex-apa).
:::
